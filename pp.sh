...
help() {
    printf '%s\n' \
        ...
        '  (applied order) [-trim start:end] [-crop w:h[:x:y]] [-scale w:h]' \
        '*) FFmpeg filters the first video stream and its metadata, copying chapters.' \
        '*) Uncompressed frames or filter string are streamed to /dev/stdout.' \
        '*) For av1an, use "-f -vf $(pp.sh video.mp4 -av1an ...)".'
    exit $1
}
...
cli() {
    ...
    video="$1" format=yuv4mpegpipe trim='' crop='' scale='' ffmpeg='' ffprobe=''; shift
    while [ $# -gt 0 ]; do
        case "$1" in
